...
| --- | --- |
| A ticket on the tracker | `/implement #42`, one ticket per [session](https://michalkow.github.io/skills/dictionary/session), [clearing](https://michalkow.github.io/skills/dictionary/clearing) context between tickets |
...
The same-session case is worth naming because the skill's own first line doesn't cover it. `SKILL.md` says "the spec or tickets", which nudges the [model](https://michalkow.github.io/skills/dictionary/model) to go hunting for a file that doesn't exist. If the plan lives only in the thread, say so when you invoke it.
...
One run covers one ticket. The tickets [to-tickets](https://michalkow.github.io/skills/to-tickets) produces are tracer-bullet vertical slices sized to fit a single fresh [context window](https://michalkow.github.io/skills/dictionary/context-window), so the intended rhythm is: clear context, implement one ticket, commit, clear again. Each ticket is self-contained, which is what makes the previous ticket's context disposable.
...
Probably the ticket is too big rather than the skill being misused. A run does codebase exploration, a red-green loop per seam, a full suite, and a review, so a non-trivial ticket exceeding 100k [tokens](https://michalkow.github.io/skills/dictionary/token) is normal rather than a sign something broke. The lever is upstream: right-size the tickets in [to-tickets](https://michalkow.github.io/skills/to-tickets) so each fits one fresh window. If a single ticket keeps blowing out, split it rather than raising the [effort](https://michalkow.github.io/skills/dictionary/effort) level.
