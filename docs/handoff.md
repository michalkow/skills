...
`handoff` compacts the conversation you are in into a **handoff document** — one markdown file, written to your OS's temporary directory rather than into the workspace, that a fresh [agent](https://michalkow.github.io/dictionary-of-ai-coding/agent) can read to pick the work up.

What it buys is **portability**, not compression. That makes the skill narrower than it sounds. You need a file only when the work has to *travel* — to a new [harness](https://michalkow.github.io/dictionary-of-ai-coding/harness), a new directory, a colleague, or a side task you want to fork off. If nothing is travelling, you do not need a handoff: staying in the [session](https://michalkow.github.io/dictionary-of-ai-coding/session), `/clear`, a [subagent](https://michalkow.github.io/dictionary-of-ai-coding/subagent) and `/compact` cover the ordinary end-of-phase case, and `/compact` covers it more often than this skill does.
...
| --- | --- |
| Swapping harness — Claude → Codex | The new harness cannot see the old [context](https://michalkow.github.io/dictionary-of-ai-coding/context) |
...
For anything else — same harness, same directory, you are done [grilling](https://michalkow.github.io/dictionary-of-ai-coding/grilling) and moving to implementation — `/compact` is the move. [ask-michal](https://michalkow.github.io/skills/ask-michal) carries the ordered tree over all five options at a phase boundary.
...
Three different things being preserved. `/compact` compresses this context and keeps you going in a fresh window — intent survives. `/clear` empties the window and starts from nothing — correct when everything behind you is disposable, and one-way if it isn't. `/handoff` writes a portable file — the work survives the move to somewhere else. Note that all three turn a **[primary source](https://michalkow.github.io/dictionary-of-ai-coding/primary-source)** (the conversation as it happened) into a **[secondary source](https://michalkow.github.io/dictionary-of-ai-coding/secondary-source)** (a summary of it). Continuing is the only move that doesn't, which is why it's the first one to rule out.
...
`handoff` is a **reach-for-it-anytime standalone** that lives at the seam between sessions rather than inside a build chain — but a narrow one, and the honest map is that you'll use it less often than the other four options at a phase boundary. Its closest neighbour is [prototype](https://michalkow.github.io/skills/prototype), because a prototype lives in its own directory and the round trip out and back is exactly the crossing this skill is for. When you're at a boundary and unsure whether to continue, clear, hand off, delegate or compact, [ask-michal](https://michalkow.github.io/skills/ask-michal) carries the tree that orders those five — and routes you over the rest of the set.
