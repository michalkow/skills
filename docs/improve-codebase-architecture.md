...
You invoke this by typing `/improve-codebase-architecture` — the [agent](https://michalkow.github.io/dictionary-of-ai-coding/agent) will not reach for it on its own.
...
| Before a big build | Point it at the [spec](https://michalkow.github.io/dictionary-of-ai-coding/spec): "how can we make this change easy?" This is the most effective prompt for it. |
| Brownfield audit | Run it on a large, unstructured or [vibe-coded](https://michalkow.github.io/dictionary-of-ai-coding/vibe-coding) repo to find out what shape it is actually in. |
...
- For designing one module you have already chosen, use [codebase-design](https://michalkow.github.io/skills/codebase-design) — that is the bench, this is the survey that finds what to put on it.
- For a whole effort too big to hold in one session, use [wayfinder](https://michalkow.github.io/skills/wayfinder).
- For "this specific thing is broken," use [diagnosing-bugs](https://michalkow.github.io/skills/diagnosing-bugs). It hands back here when the real finding is that there is no good seam to lock the bug down.
...
Picking a candidate starts a [grilling](https://michalkow.github.io/skills/grilling) session over it: constraints, what sits behind the seam, which tests survive, what the deepened interface should look like. The output of that session is a decision, not a diff. From there the normal flow applies — take the decision into [to-spec](https://michalkow.github.io/skills/to-spec), then [to-tickets](https://michalkow.github.io/skills/to-tickets), then [implement](https://michalkow.github.io/skills/implement).

## Common questions

**It grilled me for an hour about one idea instead of showing me options. Can I turn that off?**

Yes — say so when you invoke it ("don't grill me, just show the report"). This is the loudest complaint the skill has. One user put it bluntly: they liked it as "a convenient way to get a thorough analysis of improvements," and after the grilling loop was added found it "borderline unusable," reporting sessions where it proposed a single solution and then asked "10's or 100's of questions." The design intent is that the report comes first and the grill only starts on a candidate you chose, but weaker [models](https://michalkow.github.io/dictionary-of-ai-coding/model) skip straight to interviewing you about the first idea they had. Reports in that thread vary sharply by model, and it is an open issue — the skill does not yet have a documented no-grill mode.
...
One candidate per session. Working through several in one conversation fills the [context window](https://michalkow.github.io/dictionary-of-ai-coding/context-window) with the report, the grilling, the domain-model edits and the code changes all at once. The report only lives in a temp file, so carry the candidate itself rather than the file: pick one, grill it, take the decision into `/to-spec`, and turn the rest into [tickets](https://michalkow.github.io/dictionary-of-ai-coding/ticket) you can pick up independently later. Put the chosen improvement into a spec rather than going straight to implementation. This is a recurring question with no documented workflow in the skill itself.
...
Partially. The exploration step names Claude Code's `Agent` tool with `subagent_type=Explore` directly, so a [harness](https://michalkow.github.io/dictionary-of-ai-coding/harness) without that tool may skip the parallel exploration rather than substitute its own. The skill still runs; the scan is just less thorough. A harness-neutral rewrite has been proposed but is not merged.
...
`improve-codebase-architecture` is **periodic maintenance** — run it every few days, outside any chain, to queue up work rather than to do it. Its neighbours are [codebase-design](https://michalkow.github.io/skills/codebase-design), which owns the depth-and-seam vocabulary every candidate is written in, [grilling](https://michalkow.github.io/skills/grilling), which walks the decision tree once you have chosen a candidate, and [domain-modeling](https://michalkow.github.io/skills/domain-modeling), which keeps `CONTEXT.md` and the ADRs current as the decision settles. What it produces is an idea, which re-enters the main build flow at [grill-with-docs](https://michalkow.github.io/skills/grill-with-docs) or [to-spec](https://michalkow.github.io/skills/to-spec). For which skill fits a situation, [ask-michal](https://michalkow.github.io/skills/ask-michal) is the router over the whole set.
