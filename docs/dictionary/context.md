...
The relevant information the [agent](./agent.md) has access to right now. The abstract noun — not the raw input the model sees (that's the [context window](./context-window.md)), not the running history (that's the [session](./session.md)), but _what the agent knows that's pertinent to the task_. "Loading something into context" means making it part of this set; "context engineering" is the discipline of curating it.
