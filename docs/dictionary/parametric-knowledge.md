...
Parametric knowledge also ages. The parameters stop changing at the [knowledge cutoff](./knowledge-cutoff.md), so a library released or renamed after that date doesn't exist in them, and an API that changed is remembered in its old form.
