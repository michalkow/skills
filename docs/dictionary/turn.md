...
One user message plus everything the [agent](./agent.md) does in response, up until it yields back to the user. Contains one or more [model provider requests](./model-provider-request.md) — many, if the agent calls [tools](./tool.md). A clarifying question closes the turn; your reply opens the next one. The hierarchy is [session](./session.md) **> Turn > Model provider request**.
...
"It made fourteen [tool calls](./tool-call.md) inside that turn — each one is a separate model provider request. Latency stacks up before the agent finally yields back to you."
