...
Away from keyboard. A working pattern where the user kicks off a [session](./session.md) and leaves the [agent](./agent.md) to run unattended. The throughput multiplier of [AI](./ai.md) coding — many AFK sessions can run in parallel while you sleep, eat, or work on something else. Usually requires a permissive [permission mode](./permission-mode.md) plus [sandboxing](./sandbox.md) to be safe.
...
"[Bypass permissions](./agent-mode.md)?"
