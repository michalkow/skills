...
One round-trip from the [harness](./harness.md) to the [model provider](./model-provider.md). The harness sends the current [context](./context.md); the provider returns one response (a [tool call](./tool-call.md) or a final answer). A single user message can spawn many model provider requests if the [agent](./agent.md) calls [tools](./tool.md) — each [tool result](./tool-result.md) triggers another request.

Each request carries everything: the [system prompt](./system-prompt.md), the full conversation so far, every tool result. The [model](./model.md) is [stateless](./stateless.md), so the provider keeps nothing between requests — request forty re-sends what request thirty-nine sent, plus one more tool result. The [prefix cache](./prefix-cache.md) exists to make this repetition affordable.

The request is also the unit of billing. [Input tokens](./input-tokens.md), [output tokens](./output-tokens.md), and cache discounts are all counted per request, which is why an innocuous-looking question can cost a surprising amount: the cost isn't proportional to your message, it's proportional to the number of requests times the size of the context each one carries.
