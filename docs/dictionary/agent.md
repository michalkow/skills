...
A [model](./model.md) [harnessed](./harness.md) with [tools](./tool.md), a [system prompt](./system-prompt.md), and a [context window](./context-window.md), that takes [turns](./turn.md) with a user. _Claude Code is an agent. Cursor is an agent. Claude.ai is an agent._ An agent is what you actually talk to — it's the model in motion, configured for a purpose.
