...
A teachable capability bundled as a unit — instructions and resources for doing one task well, kept in the [environment](./environment.md) until a [context pointer](./context-pointer.md) pulls it into the [context window](./context-window.md) for the task at hand. The unit of [progressive disclosure](./progressive-disclosure.md) in a [harness](./harness.md).
