...
The date past which a [model](./model.md) has no [parametric knowledge](./parametric-knowledge.md). Libraries, APIs, and events from after the cutoff are fabrication traps unless their docs are loaded as [contextual knowledge](./contextual-knowledge.md). Each model release ships with its own cutoff.
