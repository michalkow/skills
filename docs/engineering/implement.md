...
If the tickets came from [to-tickets](https://aihero.dev/skills-to-tickets), the tracker they live on was configured by [setup-michalkow-skills](https://aihero.dev/skills-setup-michalkow-skills). `code-review` reads the same configuration to find the originating spec at close-out.
...
[ask-michal](https://aihero.dev/skills-ask-michal) is the router over the whole set when you are not sure which flow you are in.
