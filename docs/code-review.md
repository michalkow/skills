...
`code-review` reviews the diff between `HEAD` and a fixed point you name — a commit, a branch, a tag, `main`, `HEAD~5` — along two axes. **Standards** asks whether the code follows how this repo writes code. **Spec** asks whether the code does what the originating issue or [spec](https://michalkow.github.io/dictionary-of-ai-coding/spec) asked for. Each axis runs in its own [sub-agent](https://michalkow.github.io/dictionary-of-ai-coding/subagent) so neither sees the other's reasoning.

The two axes are never merged and never re-ranked. The report ends with a worst issue *per axis* and refuses to name a single winner across them, because a change can pass one axis and fail the other: code that follows every convention while implementing the wrong thing passes Standards and fails Spec; code that does exactly what the [ticket](https://michalkow.github.io/dictionary-of-ai-coding/ticket) asked while breaking the repo's conventions does the reverse. A blended verdict lets the passing axis hide the failing one.
...
| Nothing is written yet and you want it written test-first | [tdd](https://michalkow.github.io/skills/tdd) |
| A whole spec needs building, review included | [implement](https://michalkow.github.io/skills/implement), which calls this skill itself |
| The whole codebase has drifted, not one diff | [improve-codebase-architecture](https://michalkow.github.io/skills/improve-codebase-architecture) |
| Something is broken and you do not know why | [diagnosing-bugs](https://michalkow.github.io/skills/diagnosing-bugs) |
...
Step 1 depends on `docs/agents/issue-tracker.md`, which [setup-michalkow-skills](https://michalkow.github.io/skills/setup-michalkow-skills) writes. Without it the axis still works if you hand it a path. With no spec at all, the Spec sub-agent is skipped and the report says "no spec available" rather than inventing requirements.
...
- [implement](https://michalkow.github.io/skills/implement) is the closest neighbour: it drives the build and calls this skill as its own closing review before committing.
...
[ask-michal](https://michalkow.github.io/skills/ask-michal) routes across the whole set when you are unsure which skill the situation wants.
