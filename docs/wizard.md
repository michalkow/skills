...
The [agent](https://michalkow.github.io/dictionary-of-ai-coding/agent) writes the script; it never runs it. You do, on your own machine. So a wizard is not a list of instructions you follow — it is a program that drives the procedure and holds the state, and your part is to click, paste, and press Enter.
...
Don't reach for it to *decide* what to build; for that, [grill-with-docs](https://michalkow.github.io/skills/grill-with-docs) and [to-spec](https://michalkow.github.io/skills/to-spec) are the tools.
...
Scoping happens before a line is written. The [skill](https://michalkow.github.io/dictionary-of-ai-coding/skill) reads the repo instead of asking cold: `.env*`, `docker-compose*`, framework config, and every `secrets.*` / `vars.*` reference in `.github/workflows/` — each of those is a value the wizard has to produce. It then shows you the ordered stage list to confirm, and only after that maps each stage to the exact path a human follows ("Dashboard → Developers → API keys → Reveal test key → copy"). Where it doesn't know the current UI, it asks you or checks the docs rather than inventing clicks.
...
The artifact does, unconditionally: it's a plain bash script and it doesn't care what [harness](https://michalkow.github.io/dictionary-of-ai-coding/harness) generated it. The skill itself is model-invoked, so it's listed everywhere — type `/wizard` in Claude Code or `$wizard` in Codex, or just describe the setup you're stuck on. Being model-invoked also keeps it clear of [#693](https://github.com/mattpocock/skills/issues/693), where Claude's desktop and web surfaces drop *user-invoked* skills from the [model](https://michalkow.github.io/dictionary-of-ai-coding/model)'s listing and report them as not installed.
...
`wizard` is a reach-for-it-anytime standalone, sitting at the line where automation stops and a human has to click. Its nearest neighbour is [setup-michalkow-skills](https://michalkow.github.io/skills/setup-michalkow-skills), because both exist to get a repo into a working state — that one configures this skill set, while `wizard` generates a setup path for everything else. It also pairs with [implement](https://michalkow.github.io/skills/implement): when a build lands a feature that needs credentials or a manual cutover, a wizard is how the human half gets done. When you're unsure which skill fits the moment, [ask-michal](https://michalkow.github.io/skills/ask-michal) routes you.
