...
`wayfinder` takes an effort too big for one agent [session](https://michalkow.github.io/dictionary-of-ai-coding/session) — an idea whose **destination** you can name but whose route you cannot yet see — and charts it as a shared **map** of **decision tickets** on your issue tracker, then resolves them one at a time until the way is clear.

It plans, it does not do. Every ticket holds a question whose resolution is a decision, not a slice of a build to execute, and the map is finished when nothing is left to decide before someone goes and builds the thing. That one rule is what separates a wayfinder ticket from an ordinary implementation [ticket](https://michalkow.github.io/dictionary-of-ai-coding/ticket), and it is the rule agents break most often. When the map clears, wayfinder hands off; it does not carry on into code.

## When to reach for it

You invoke this by typing `/wayfinder` — the [agent](https://michalkow.github.io/dictionary-of-ai-coding/agent) won't reach for it on its own.
...
| --- | --- |
| A well-scoped feature you can settle in one sitting | [grill-me](https://michalkow.github.io/skills/grill-me), or [grill-with-docs](https://michalkow.github.io/skills/grill-with-docs) when there is a codebase |
| A greenfield project, or a build spanning many sessions, with the route still unclear | `/wayfinder` |
| A thread where the deciding is already done | [to-spec](https://michalkow.github.io/skills/to-spec) — skip straight past the map |
| A cleared wayfinder map | [to-spec](https://michalkow.github.io/skills/to-spec), then [to-tickets](https://michalkow.github.io/skills/to-tickets) and [implement](https://michalkow.github.io/skills/implement) |
| An existing session that has already grown too big | say "hand off to `/wayfinder`" — [handoff](https://michalkow.github.io/skills/handoff) bridges into a map as well as out of one |
...
The map and its tickets live on the repo's issue tracker, so wayfinder needs the tracker wiring that [setup-michalkow-skills](https://michalkow.github.io/skills/setup-michalkow-skills) lays down. That step writes a "Wayfinding operations" section describing how the map, its child tickets, blocking edges, and frontier queries are expressed for GitHub, GitLab, Linear, or local markdown. Wayfinder resolves that doc through the pointer in your `CLAUDE.md` / `AGENTS.md` rather than a fixed path; with no tracker configured at all it falls back to local markdown files.
...
Every ticket carries a `wayfinder:<type>` label, and is either **[HITL](https://michalkow.github.io/dictionary-of-ai-coding/human-in-the-loop)** — worked with a human who speaks for themselves — or **[AFK](https://michalkow.github.io/dictionary-of-ai-coding/afk)**, driven by the agent alone. A HITL ticket only resolves through the live exchange; an agent that answers its own [grilling](https://michalkow.github.io/dictionary-of-ai-coding/grilling) questions has broken it.

| Type | Mode | Reach for it when | Resolved by |
| --- | --- | --- | --- |
| `grilling` | HITL | The default. The question can be settled by talking it through. | [grilling](https://michalkow.github.io/skills/grilling) plus [domain-modeling](https://michalkow.github.io/skills/domain-modeling), in a fresh session |
| `prototype` | HITL | "How should this look" or "how should this behave" — a question talking cannot settle. | [prototype](https://michalkow.github.io/skills/prototype), with the built artifact linked from the ticket as an asset |
| `research` | AFK | A fact outside the working directory is blocking a decision. | A [research](https://michalkow.github.io/skills/research) [subagent](https://michalkow.github.io/dictionary-of-ai-coding/subagent), fired at charting time and burned down in parallel on a `research/<name>` branch |
...
The whole map — the destination of the entire map, not just the initial session. The question reads ambiguously because wayfinder is by definition a multi-session tool, so a session-scoped answer never makes sense. Typical destinations are a [spec](https://michalkow.github.io/dictionary-of-ai-coding/spec) to hand off, a decision to lock before planning starts, a proof of concept, or a change made in place like a data migration.
...
This is the sharpest live complaint about wayfinder and it is not resolved. The decomposition one user gave: the verbosity itself causes decision exhaustion, and the length strips out *why* a question is being asked, so you lose the chain from decision to decision as the map gets longer. The verbosity looks like a property of the current set of [models](https://michalkow.github.io/dictionary-of-ai-coding/model) rather than of the skill, and no fix has landed. Practitioner mitigations in circulation: run a lower [reasoning effort](https://michalkow.github.io/dictionary-of-ai-coding/effort), and put a plain-language instruction in your global `CLAUDE.md`. Expect to spend real thought here regardless — the amount of thinking wayfinder demands from you is not a defect, it is most of what it is for.
...
`wayfinder` is a **situational on-ramp**, not the default front door. The grill-led idea → ship chain is still where most work starts; wayfinder is what you climb onto when the idea is too big to hold in one session, and it merges back onto that chain at [to-spec](https://michalkow.github.io/skills/to-spec), because a cleared map hands off rather than builds.

Underneath, it is mostly other skills wearing wayfinder's scheduling: [grilling](https://michalkow.github.io/skills/grilling) and [domain-modeling](https://michalkow.github.io/skills/domain-modeling) resolve the default ticket type, [prototype](https://michalkow.github.io/skills/prototype) resolves the tickets that talking cannot, and [research](https://michalkow.github.io/skills/research) runs as a subagent so its reading never lands in your session. [handoff](https://michalkow.github.io/skills/handoff) is the bridge in and out — into a map from a conversation that outgrew itself, out of one when a side quest appears mid-session. For anything else, [ask-michal](https://michalkow.github.io/skills/ask-michal) routes over the whole set.
