...
| A raw bug report from someone else, not yet confirmed or written up | [triage](https://michalkow.github.io/skills/triage) first |
| Throwaway code to answer a design question, not chase a defect | [prototype](https://michalkow.github.io/skills/prototype) |
| Building a planned behaviour test-first | [tdd](https://michalkow.github.io/skills/tdd) |
| No good seam exists to lock the bug down | [improve-codebase-architecture](https://michalkow.github.io/skills/improve-codebase-architecture) — this skill hands off there itself |
...
10. A [human-in-the-loop](https://michalkow.github.io/dictionary-of-ai-coding/human-in-the-loop) bash script, last resort. The skill ships `scripts/hitl-loop.template.sh` for this: the agent runs the script, you follow prompts in your terminal, and your answers come back as parseable output.
...
When it genuinely cannot build one, it is instructed to stop and say so, list what it tried, and ask you for [environment](https://michalkow.github.io/dictionary-of-ai-coding/environment) access, a captured artifact, or permission to add temporary instrumentation. It should not proceed to hypothesise anyway.
...
`diagnosing-bugs` is a reach-for-it-anytime standalone. You drop into it when something is broken and drop out when the fix and its regression test are in; it holds no state and needs no prior setup. [ask-michal](https://michalkow.github.io/skills/ask-michal) routes "Something's broken" here.

Two neighbours matter. [improve-codebase-architecture](https://michalkow.github.io/skills/improve-codebase-architecture) takes the [handoff](https://michalkow.github.io/dictionary-of-ai-coding/handoff) when the real finding is that the code has no seam to lock the bug down — the recommendation is made after the fix is in, when there is more information. [triage](https://michalkow.github.io/skills/triage) sits upstream of it for bugs that arrive as raw reports from other people, and does a shallower version of the same first two phases.
