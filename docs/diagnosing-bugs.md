...
10. A [human-in-the-loop](https://michalkow.github.io/skills/dictionary/human-in-the-loop) bash script, last resort. The skill ships `scripts/hitl-loop.template.sh` for this: the agent runs the script, you follow prompts in your terminal, and your answers come back as parseable output.
...
When it genuinely cannot build one, it is instructed to stop and say so, list what it tried, and ask you for [environment](https://michalkow.github.io/skills/dictionary/environment) access, a captured artifact, or permission to add temporary instrumentation. It should not proceed to hypothesise anyway.
...
Two neighbours matter. [improve-codebase-architecture](https://michalkow.github.io/skills/improve-codebase-architecture) takes the [handoff](https://michalkow.github.io/skills/dictionary/handoff) when the real finding is that the code has no seam to lock the bug down — the recommendation is made after the fix is in, when there is more information. [triage](https://michalkow.github.io/skills/triage) sits upstream of it for bugs that arrive as raw reports from other people, and does a shallower version of the same first two phases.
