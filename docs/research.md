...
`research` answers a question by reading the sources that own the answer, then leaves a cited Markdown file in the repo. It works only from **[primary sources](https://michalkow.github.io/dictionary-of-ai-coding/primary-source)** — official docs, source code, specs, first-party APIs — and follows every claim back to the source that owns it, so it will not repeat a blog post's account of an API when the API's own docs are reachable.

It does not answer you in the conversation. The output is a file, written where the repo already keeps such notes, with a link on each claim. That is the point: a document you can react to, hand to another agent, or throw away, rather than an answer that vanishes when the [session](https://michalkow.github.io/dictionary-of-ai-coding/session) ends.

## When to reach for it

Type `/research`, or the [agent](https://michalkow.github.io/dictionary-of-ai-coding/agent) reaches for it automatically when a task turns into reading legwork.
...
| A decision made *with* you, by interview | [grilling](https://michalkow.github.io/skills/grilling) |
| A durable architecture decision, written into `CONTEXT.md` and ADRs | [grill-with-docs](https://michalkow.github.io/skills/grill-with-docs) |
| To find out whether an approach works in your codebase | [prototype](https://michalkow.github.io/skills/prototype) |
| A plan too big to hold in one session | [wayfinder](https://michalkow.github.io/skills/wayfinder) |

The line between `research` and `grill-with-docs` is the **shelf life of what comes back**. Research produces short-lived assets — what this library's auth mechanism does as of this week. An ADR records a decision you keep. If what you are producing is a decision rather than a fact, you are [grilling](https://michalkow.github.io/dictionary-of-ai-coding/grilling), not researching.
...
No, it now fires them for you. In the unreleased changes since v1.1, a charting session spawns a `/research` subagent per research ticket and burns them down in parallel, capturing findings on a throwaway `research/<name>` branch with a [context pointer](https://michalkow.github.io/dictionary-of-ai-coding/context-pointer) from the ticket. Research tickets are the one exception to wayfinder's one-ticket-per-session rule, because they are [AFK](https://michalkow.github.io/dictionary-of-ai-coding/afk) — nothing waits on you. Two known snags with those branches: the subagent has been seen opening a draft PR from a branch that is never meant to merge ([issue #576](https://github.com/mattpocock/skills/issues/576)), and deleting the branch later breaks the context pointers the tickets hold.
...
A reach-for-it-anytime standalone that feeds the thinking skills rather than sitting in the build chain. Its file is something to take *into* the flow: [grilling](https://michalkow.github.io/skills/grilling) and [grill-with-docs](https://michalkow.github.io/skills/grill-with-docs) ask sharper questions when the facts are already on the table, and [to-spec](https://michalkow.github.io/skills/to-spec) can synthesise against it. [wayfinder](https://michalkow.github.io/skills/wayfinder) is the one skill that invokes it directly, resolving each research ticket on its map with a `/research` subagent. For the whole map, see [ask-michal](https://michalkow.github.io/skills/ask-michal).
