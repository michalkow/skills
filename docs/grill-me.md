...
`grill-me` takes a **loose idea** and interviews you until you can commit to it. You do not need a worked-out plan to start — producing one is what the [session](https://michalkow.github.io/dictionary-of-ai-coding/session) is for. It asks in **rounds**: each round is the whole **frontier** — every question whose prerequisites you have already settled — so you are never asked something that hinges on an answer it hasn't heard yet.

It is **[stateless](https://michalkow.github.io/dictionary-of-ai-coding/stateless)**. It writes no files and leaves no workspace behind. The only thing it leaves is a sharper version of the idea, in your own head.

## When to reach for it

You invoke this by typing `/grill-me` — the [agent](https://michalkow.github.io/dictionary-of-ai-coding/agent) won't reach for it on its own. Start it in a **fresh conversation**, not on top of a plan you already had an agent write.
...
- **A codebase to align against** — [grill-with-docs](https://michalkow.github.io/skills/grill-with-docs). The same interview, but [stateful](https://michalkow.github.io/dictionary-of-ai-coding/stateful): it reads your code and keeps what it learns in `CONTEXT.md` and ADRs.
...
Leave [plan mode](https://michalkow.github.io/dictionary-of-ai-coding/agent-mode) off. Plan mode primes the agent to rush toward producing a plan, which is the opposite of staying in inquiry.
...
"One long form or three pages?" and "how should this interaction feel?" are **ungrillable** — they need something to react to. When you hit one, stop grilling. Build the throwaway version with [prototype](https://michalkow.github.io/skills/prototype), look at it, then come back and answer in one line.
...
Usually the scope was too large. Ask the agent to break the work into smaller pieces first, then grill each one. Very long sessions also drift into the **[dumb zone](https://michalkow.github.io/dictionary-of-ai-coding/smart-zone)**, where the [context window](https://michalkow.github.io/dictionary-of-ai-coding/context-window) is full enough that the questions get worse.
...
No. The value of the session is the [context](https://michalkow.github.io/dictionary-of-ai-coding/context) you just built. Hand the same conversation straight to [to-spec](https://michalkow.github.io/skills/to-spec).

**Does the model matter?**
More than for most skills. Grilling leans on the [model](https://michalkow.github.io/dictionary-of-ai-coding/model)'s own sense of how systems break, so give it your best one. Implementation mostly follows context and tolerates a cheaper model.
